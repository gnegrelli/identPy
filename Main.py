...
if not master_slave:
    ...
    op = MS_rk4(init_time,final_time,step,x_o,p,delta_p,u,-1,op_real_ms[:,1])


#Error Evaluation (Least-Mean Square)
Jp = .5*step*Error(op_real,op)
err = Jp          #Variable Storing Error Evolution


if print_figs:    
    f, axarr = plt.subplots(2, sharex=True)
    axarr[0].plot(op_real[:,0],op_real[:,1],op[:,0],op[:,1])
    axarr[0].set_title('Theta')
    axarr[0].set_ylabel("Angle (rad)")
    axarr[0].set_xlabel("Time (s)")
    
    axarr[1].plot(op_real[:,0],op_real[:,2],op[:,0],op[:,2])
...
while Jp > tolerance and count < 50:
    ...
    for i in range(0,num_param):
        if not master_slave:
            ...
            op_p = MS_rk4(init_time,final_time,step,x_o,p,delta_p,u,i,op_real_ms[:,1])
