...
from identpy.objects import ImplicitMethod as IM


class RK4(IM):

    def __init__(self, initial_time=0, final_time=1, step=0.001):
        assert isinstance(initial_time, (float, int)), "Initial time must be a number"
        assert isinstance(final_time, (float, int)), "Final time must be a number"
        assert isinstance(step, (float, int)), "Step must be a number"
